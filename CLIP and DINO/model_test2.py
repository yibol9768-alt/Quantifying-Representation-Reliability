...
def evaluate_model():
    ...
    # 加载测试集特征
    print("正在加载测试集特征")
    features_dict = torch.load("multiview_test_features.pt", map_location="cpu")
    ...
    model.load_state_dict(torch.load("multiview_fusion_mlp.pth", map_location=device))
    ...
    print("-" * 40)
    print(f"多视图融合模型 (CLIP + DINO) 最终成绩")
