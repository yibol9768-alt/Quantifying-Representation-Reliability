...
def get_extractor(
    ...
    if fusion_method == "bilinear_concat":
        return MultiModelBilinearExtractor(
            model_types,
            proj_dim=fusion_kwargs.get("proj_dim", 64),
            model_dir=model_dir,
        )
    if fusion_method == "film":
        return MultiModelFiLMExtractor(
            model_types,
            proj_dim=fusion_kwargs.get("proj_dim", 512),
            model_dir=model_dir,
        )
    if fusion_method == "context_gating":
        return MultiModelContextGatingExtractor(
            model_types,
            proj_dim=fusion_kwargs.get("proj_dim", 256),
            model_dir=model_dir,
        )
    if fusion_method == "lmf":
        return MultiModelLMFExtractor(
            model_types,
            proj_dim=fusion_kwargs.get("proj_dim", 512),
            rank=fusion_kwargs.get("lmf_rank", 16),
            output_dim=fusion_kwargs.get("lmf_output_dim", 512),
            model_dir=model_dir,
        )
    if fusion_method == "se_fusion":
        return MultiModelSEFusionExtractor(
            model_types,
            proj_dim=fusion_kwargs.get("proj_dim", 512),
            reduction=fusion_kwargs.get("se_reduction", 4),
            model_dir=model_dir,
        )
    ...
    raise ValueError(
        ...
        f"'difference_concat', 'hadamard_concat', 'bilinear_concat', "
        f"'film', 'context_gating', 'lmf', 'se_fusion', "
        f"'late_fusion', 'comm', 'mmvit', "
