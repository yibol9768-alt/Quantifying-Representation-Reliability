...
def _build_dataset(name: str, root: str, split: str) -> Dataset:
    ...
    if name == "gtsrb":
        ...
        # 数据在 root/svhn/ 子目录下
        return tv_datasets.SVHN(os.path.join(root, "svhn"),
        ...
        # 数据在 root/pets/oxford-iiit-pet/ 子目录下
        return tv_datasets.OxfordIIITPet(
            os.path.join(root, "pets"), split="trainval" if train else "test",
            target_types="category", transform=TRAIN_TRANSFORM, download=False,
        )
    elif name == "country211":
        # 数据在 root/country211/country211/ 子目录下
        return tv_datasets.Country211(os.path.join(root, "country211"),
                                       split="train" if train else "test",
                                       transform=TRAIN_TRANSFORM, download=False)
    elif name == "imagenet":
        # 使用已解压的 train 目录（同时作为 train 和 test，类似 EuroSAT）
        train_dir = os.path.join(root, "imagenet", "train")
        return tv_datasets.ImageFolder(train_dir, transform=TRAIN_TRANSFORM)
    else:
