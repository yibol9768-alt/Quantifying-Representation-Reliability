...
def extract_test_features():
    ...
    with torch.no_grad():
        for img_path, label in tqdm(zip(image_paths, labels), total=len(image_paths)):
            try:
                ...
                mae_inputs = mae_processor(images=img, return_tensors="pt").to(device)
                ...
                mae_feat = mae_outputs.last_hidden_state[:, 0, :]
                mae_feat = mae_feat / mae_feat.norm(dim=-1, keepdim=True)
    ...
    features_dict = {
        ...
        "mae_features": torch.cat(mae_features_list, dim=0),
        "labels": torch.tensor(labels_list, dtype=torch.long)
    }
    
    torch.save(features_dict, "multiview3_test_features.pt")
    print("测试集多视图特征已成功保存至 multiview3_test_features.pt！")
