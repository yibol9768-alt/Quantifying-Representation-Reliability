"""
Training script

Usage:
    # Single model
    python scripts/train.py --model clip --dataset cifar10
    
    # Multi-model concat fusion
    python scripts/train.py --models clip dino --dataset cifar10
    
    # COMM fusion
    python scripts/train.py --method comm --dataset cifar10
    
    # COMM3 fusion
    python scripts/train.py --method comm3 --dataset cifar10
"""
import argparse
import os
import sys

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import torch
import torch.nn as nn
from torch.utils.data import DataLoader, TensorDataset
from tqdm import tqdm


def train_single(args):
    """Train single model classifier"""
    from src.training import SingleViewClassifier
    from src.data import DATASET_INFO
    from configs.config import MODEL_CONFIGS
    
    # Load features
    train_data = torch.load(f"features/{args.dataset}_{args.model}_train.pt")
    test_data = torch.load(f"features/{args.dataset}_{args.model}_test.pt")
    
    num_classes = DATASET_INFO[args.dataset]["num_classes"]
    feature_dim = MODEL_CONFIGS[args.model]["feature_dim"]
    
    model = SingleViewClassifier(feature_dim=feature_dim, num_classes=num_classes)
    return train_model(model, train_data, test_data, args)


def train_concat(args):
    """Train multi-model concat fusion classifier"""
    from src.training import MultiViewClassifier
    from src.data import DATASET_INFO
    from configs.config import MODEL_CONFIGS
    
    model_str = "_".join(args.models)
    
    # Load features
    train_data = torch.load(f"features/{args.dataset}_{model_str}_train.pt")
    test_data = torch.load(f"features/{args.dataset}_{model_str}_test.pt")
    
    num_classes = DATASET_INFO[args.dataset]["num_classes"]
    feature_dims = [MODEL_CONFIGS[m]["feature_dim"] for m in args.models]
    
    model = MultiViewClassifier(feature_dims=feature_dims, num_classes=num_classes)
    return train_model(model, train_data, test_data, args)


def train_comm(args):
    """Train COMM fusion classifier"""
    from src.training import COMMFusionClassifier
    from src.data import DATASET_INFO
    
    # Load features
    train_data = torch.load(f"features/{args.dataset}_comm_train.pt")
    test_data = torch.load(f"features/{args.dataset}_comm_test.pt")
    
    num_classes = DATASET_INFO[args.dataset]["num_classes"]
    
    model = COMMFusionClassifier(
        clip_hidden_dim=768, clip_output_dim=512, clip_num_layers=12,
        dino_hidden_dim=768, dino_num_layers=6,
        num_classes=num_classes
    )
    return train_comm_model(model, train_data, test_data, args)


def train_comm3(args):
    """Train COMM3 fusion classifier"""
    from src.training import COMM3FusionClassifier
    from src.data import DATASET_INFO
    
    # Load features
    train_data = torch.load(f"features/{args.dataset}_comm3_train.pt")
    test_data = torch.load(f"features/{args.dataset}_comm3_test.pt")
    
    num_classes = DATASET_INFO[args.dataset]["num_classes"]
    
    model = COMM3FusionClassifier(
        clip_hidden_dim=768, clip_output_dim=512, clip_num_layers=12,
        dino_hidden_dim=768, dino_num_layers=6,
        mae_hidden_dim=768, mae_num_layers=6,
        num_classes=num_classes
    )
    return train_comm3_model(model, train_data, test_data, args)


def train_model(model, train_data, test_data, args):
    """Generic training loop for standard classifiers"""
    device = args.device
    model = model.to(device)
    
    # Prepare data
    train_X = train_data["features"].float().to(device)
    train_y = train_data["labels"].long().to(device)
    test_X = test_data["features"].float().to(device)
    test_y = test_data["labels"].long().to(device)
    
    train_loader = DataLoader(TensorDataset(train_X, train_y), batch_size=args.batch_size, shuffle=True)
    
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-4)
    
    best_acc = 0
    best_state = None
    
    for epoch in range(args.epochs):
        model.train()
        for X, y in train_loader:
            optimizer.zero_grad()
            loss = criterion(model(X), y)
            loss.backward()
            optimizer.step()
        
        # Evaluate
        model.eval()
        with torch.no_grad():
            pred = model(test_X).argmax(dim=1)
            acc = (pred == test_y).float().mean().item() * 100
        
        if acc > best_acc:
            best_acc = acc
            best_state = model.state_dict().copy()
        
        print(f"Epoch {epoch+1:2d}/{args.epochs} | Test Acc: {acc:.2f}% | Best: {best_acc:.2f}%")
    
    # Save
    model.load_state_dict(best_state)
    os.makedirs("outputs/checkpoints", exist_ok=True)
    path = f"outputs/checkpoints/{args.dataset}_{args.model if hasattr(args, 'model') and args.model else '_'.join(args.models)}_{args.method if hasattr(args, 'method') and args.method else 'concat'}.pth"
    torch.save(best_state, path)
    print(f"\nSaved to: {path}")
    return best_acc


def train_comm_model(model, train_data, test_data, args):
    """Training loop for COMM classifier"""
    device = args.device
    model = model.to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-4)
    
    best_acc = 0
    best_state = None
    
    for epoch in range(args.epochs):
        model.train()
        
        # Get batch
        indices = torch.randperm(len(train_data["labels"]))
        
        for i in range(0, len(indices), args.batch_size):
            batch_idx = indices[i:i+args.batch_size]
            
            # Prepare CLIP features
            clip_feats = [train_data[f"clip_layer_{j}"][batch_idx].float().to(device) for j in range(12)]
            # Prepare DINO features
            dino_feats = [train_data[f"dino_layer_{j}"][batch_idx].float().to(device) for j in range(6, 12)]
            labels = train_data["labels"][batch_idx].long().to(device)
            
            optimizer.zero_grad()
            loss = criterion(model(clip_feats, dino_feats), labels)
            loss.backward()
            optimizer.step()
        
        # Evaluate
        model.eval()
        with torch.no_grad():
            clip_feats = [test_data[f"clip_layer_{j}"].float().to(device) for j in range(12)]
            dino_feats = [test_data[f"dino_layer_{j}"].float().to(device) for j in range(6, 12)]
            labels = test_data["labels"].long().to(device)
            
            pred = model(clip_feats, dino_feats).argmax(dim=1)
            acc = (pred == labels).float().mean().item() * 100
        
        if acc > best_acc:
            best_acc = acc
            best_state = model.state_dict().copy()
        
        print(f"Epoch {epoch+1:2d}/{args.epochs} | Test Acc: {acc:.2f}% | Best: {best_acc:.2f}%")
    
    model.load_state_dict(best_state)
    os.makedirs("outputs/checkpoints", exist_ok=True)
    path = f"outputs/checkpoints/{args.dataset}_comm.pth"
    torch.save(best_state, path)
    print(f"\nSaved to: {path}")
    return best_acc


def train_comm3_model(model, train_data, test_data, args):
    """Training loop for COMM3 classifier"""
    device = args.device
    model = model.to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=1e-4)
    
    best_acc = 0
    best_state = None
    
    for epoch in range(args.epochs):
        model.train()
        
        indices = torch.randperm(len(train_data["labels"]))
        
        for i in range(0, len(indices), args.batch_size):
            batch_idx = indices[i:i+args.batch_size]
            
            clip_feats = [train_data[f"clip_layer_{j}"][batch_idx].float().to(device) for j in range(12)]
            dino_feats = [train_data[f"dino_layer_{j}"][batch_idx].float().to(device) for j in range(6, 12)]
            mae_feats = [train_data[f"mae_layer_{j}"][batch_idx].float().to(device) for j in range(6, 12)]
            labels = train_data["labels"][batch_idx].long().to(device)
            
            optimizer.zero_grad()
            loss = criterion(model(clip_feats, dino_feats, mae_feats), labels)
            loss.backward()
            optimizer.step()
        
        # Evaluate
        model.eval()
        with torch.no_grad():
            clip_feats = [test_data[f"clip_layer_{j}"].float().to(device) for j in range(12)]
            dino_feats = [test_data[f"dino_layer_{j}"].float().to(device) for j in range(6, 12)]
            mae_feats = [test_data[f"mae_layer_{j}"].float().to(device) for j in range(6, 12)]
            labels = test_data["labels"].long().to(device)
            
            pred = model(clip_feats, dino_feats, mae_feats).argmax(dim=1)
            acc = (pred == labels).float().mean().item() * 100
        
        if acc > best_acc:
            best_acc = acc
            best_state = model.state_dict().copy()
        
        print(f"Epoch {epoch+1:2d}/{args.epochs} | Test Acc: {acc:.2f}% | Best: {best_acc:.2f}%")
    
    model.load_state_dict(best_state)
    os.makedirs("outputs/checkpoints", exist_ok=True)
    path = f"outputs/checkpoints/{args.dataset}_comm3.pth"
    torch.save(best_state, path)
    print(f"\nSaved to: {path}")
    return best_acc


def main():
    parser = argparse.ArgumentParser(description="Training")
    
    # Method selection
    method_group = parser.add_mutually_exclusive_group(required=True)
    method_group.add_argument("--model", type=str, choices=["clip", "dino", "mae"])
    method_group.add_argument("--models", type=str, nargs="+", choices=["clip", "dino", "mae"])
    method_group.add_argument("--method", type=str, choices=["comm", "comm3"])
    
    parser.add_argument("--dataset", type=str, required=True)
    parser.add_argument("--epochs", type=int, default=30)
    parser.add_argument("--batch-size", type=int, default=256)
    parser.add_argument("--lr", type=float, default=1e-3)
    parser.add_argument("--device", type=str, default="cuda")
    
    args = parser.parse_args()
    
    if args.model:
        print(f"Training {args.model.upper()} on {args.dataset}")
        train_single(args)
    elif args.models:
        print(f"Training {'+'.join(args.models).upper()} concat fusion on {args.dataset}")
        train_concat(args)
    elif args.method == "comm":
        print(f"Training COMM fusion on {args.dataset}")
        train_comm(args)
    elif args.method == "comm3":
        print(f"Training COMM3 fusion on {args.dataset}")
        train_comm3(args)


if __name__ == "__main__":
    main()
