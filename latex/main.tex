...
本节只保留当前已经完成的结果展示。\input{generated/current_selection_progress_note.tex}

\subsection{主方法在 $k=1,\dots,10$ 下的结果}

表~\ref{tab:ours-k-1-to-10} 汇总了当前已完成数据集上，主方法 Ours (LogME+CKA) 在 $k=1,\dots,10$ 下的测试准确率。这里的 $k$ 表示按排序依次取前 $k$ 个模型进入融合器，因此该表直接回答“模型数增加后，性能如何变化”。

\input{generated/ours_k_1_to_10_table.tex}

\paragraph{实验结论}
从表~\ref{tab:ours-k-1-to-10} 可以看到，性能并不会随着 $k$ 单调上升。当前四个已完成数据集上的最优 $k$ 分别为 STL10 上的 $7$、Pets 上的 $1$、EuroSAT 上的 $6$ 和 DTD 上的 $7$。这说明在冻结特征融合设定下，简单地持续增加模型数并不能保证更高的测试准确率；更合理的结论是，不同数据集通常存在各自的最优模型规模，而最优点往往出现在中等规模的前缀子集，而不是固定地取到最大 $k$。
